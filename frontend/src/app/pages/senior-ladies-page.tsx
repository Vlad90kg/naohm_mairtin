...
const FALLBACK_CONTENT = {
  title: 'Senior Ladies',
  subtitle: 'Senior ladies squads and team information managed from CMS.',
  eyebrow: 'Senior Ladies',
  intro: 'Add, remove, and update senior ladies teams in the Team CMS. Team cards and details are shown directly on this page.',
};

export function SeniorLadiesPage() {
    ...
    const loadPage = async () => {
      try {
        const [teamsData, content] = await Promise.all([
          fetchTeams({ category: 'adult', senior_group: 'senior_ladies', internal: 'true' }),
          fetchContentPageBySlug('senior-ladies').catch(() => null),
        ]);

        setTeams(teamsData);
        setContentPage(content);
      } catch (error) {
        console.error('Failed to load senior ladies page:', error);
      } finally {
        setIsLoading(false);
      }
    };
